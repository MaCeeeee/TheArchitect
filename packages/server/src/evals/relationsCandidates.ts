...
import { referencesLaw, normalizeArticleNumber } from '@thearchitect/shared';

// THE-433 (Slice 1, Task 2): Die Verweis-Muster + der Verweis-Miner
// (LAW_FAMILY_PATTERNS, SOURCE_TO_FAMILY, referencesLaw, hasReferencePatterns,
// normalizeArticleNumber) sind nach @thearchitect/shared umgezogen
// (shared/src/relations/lawPatterns.ts, mitsamt ihrer Begründungs-Kommentare):
// Eval-Harness (hier) und Crawler-Batch (Server B) müssen dieselbe
// Kandidaten-Logik ausführen — eine Wahrheit statt zwei driftender Kopien
// (dasselbe Muster wie der Typing-Prompt, shared/src/typing/prompt.ts).
// Re-Export, damit bestehende Importeure (Tests, build-relations-golden.ts)
...
export type { LawReferenceMatch } from '@thearchitect/shared';
