    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-2 border-b border-[var(--border-subtle)] flex items-center gap-2">
        ...
        <select
          ...
          className="flex-1 bg-[var(--surface-base)] border border-[var(--border-subtle)] rounded text-[10px] text-[var(--text-secondary)] px-2 py-1 outline-none"
        >
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex items-center justify-center p-6">
            <span className="text-xs text-[var(--text-tertiary)]">No audit entries found</span>
          </div>
        ) : (
          logs.map((log) => (
            <div key={log._id} className="px-3 py-2 border-b border-[#111111] hover:bg-[var(--surface-raised)]/50">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[var(--text-secondary)]">{getUserName(log.userId)}</span>
                {log.entityId && (
                  <span className="text-[9px] text-[var(--text-disabled)] font-mono">{log.entityId.slice(0, 8)}</span>
                )}
              </div>
            </div>
          ))
      ...
      {total > limit && (
        <div className="p-2 border-t border-[var(--border-subtle)] flex items-center justify-between">
          <button
            ...
            disabled={offset === 0}
            className="text-[10px] text-[var(--text-tertiary)] hover:text-white disabled:opacity-30 flex items-center gap-0.5"
          >
