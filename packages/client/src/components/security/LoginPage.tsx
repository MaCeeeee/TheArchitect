...
export default function LoginPage() {
  ...
  // OAuth provider availability — hides buttons whose env vars are missing on
  // the server, so users never click a button that will silently fail.
  ...
  useEffect(() => {
    api.get('/auth/oauth/providers')
      ...
      .catch(() => {
        // If the endpoint is unreachable, keep all hidden — better than showing
        // a button that will fail. Email login remains available.
      });
  }, []);
  const hasAnyOauth = oauthProviders.google || oauthProviders.github || oauthProviders.microsoft;
    ...
    <>
          ...
          {(mode === 'login' || mode === 'register') && (
            <>
              ...
              {/* OAuth section — only renders providers that the server confirmed are configured. */}
              {hasAnyOauth && (
                <>
                  {/* Divider */}
                  ...
                  <div className={`grid gap-2 ${
                    [oauthProviders.google, oauthProviders.github, oauthProviders.microsoft].filter(Boolean).length === 1
                      ? 'grid-cols-1'
                      : [oauthProviders.google, oauthProviders.github, oauthProviders.microsoft].filter(Boolean).length === 2
                        ? 'grid-cols-2'
                        : 'grid-cols-3'
                  }`}>
