  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="p-3 border-b border-[#334155]">
        ...
        <p className="text-[10px] text-[#64748b] mt-1">TCO estimation and optimization opportunities</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 p-3">
        <div className="rounded-md border border-[#334155] bg-[#0f172a] p-2">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] text-[#64748b]">Total TCO</span>
          </div>
        ...
        <div className="rounded-md border border-[#334155] bg-[#0f172a] p-2">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] text-[#64748b]">Save Potential</span>
          </div>
          ...
          {Object.entries(costData.byDomain)
            ...
            .map(([domain, cost]) => (
              <div key={domain} className="flex items-center gap-2">
                <span className="text-[10px] text-[#94a3b8] w-20 capitalize">{domain}</span>
                <div className="flex-1 h-3 rounded-full bg-[#0f172a]">
                  <div
                    className="h-full rounded-full"
                    style={{
                      width: `${(cost / costData.totalCost) * 100}%`,
                      backgroundColor: domainColors[domain] || '#64748b',
                    }}
                  />
                </div>
      ...
      <div className="px-3 pb-3 border-t border-[#334155] pt-3">
        <h4 className="text-[10px] font-semibold uppercase text-[#64748b] mb-2">By Lifecycle Status</h4>
        <div className="grid grid-cols-2 gap-1">
          {Object.entries(costData.byStatus).map(([status, cost]) => (
            <div key={status} className="rounded-md border border-[#334155] bg-[#0f172a] p-2" style={{ borderLeftColor: statusColors[status] || '#64748b', borderLeftWidth: 2 }}>
        ...
        <div className="px-3 pb-3 border-t border-[#334155] pt-3">
          ...
          <div className="space-y-1">
            {costData.items
              ...
              .slice(0, 8)
              .map((item) => (
                <div key={item.id} className="flex items-center gap-2 py-1 px-1 rounded hover:bg-[#1e293b]">
      ...
      {elements.length === 0 && (
        <div className="flex-1 flex items-center justify-center p-6">
          <p className="text-xs text-[#64748b] text-center">No elements for cost estimation</p>
        </div>
      )}
    </div>
  );
