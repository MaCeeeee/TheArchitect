...
export default function ApiKeysSection() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[#64748b] mb-6">Create and manage personal access tokens for API access.</p>
      ...
      {newKey && (
        <div className="mb-6 rounded-lg border border-green-600/50 bg-green-900/20 p-4">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 rounded bg-[#0f172a] px-3 py-2 text-sm text-white font-mono break-all">{newKey}</code>
            <button onClick={handleCopy} className="text-[#94a3b8] hover:text-white transition p-2">
              {copied ? <Check size={16} className="text-green-400" /> : <Copy size={16} />}
            </button>
          </div>
          <button onClick={() => setNewKey('')} className="mt-2 text-xs text-[#64748b] hover:text-[#94a3b8]">
            Dismiss
          </button>
        </div>
      )}
      ...
      <button
        ...
        className="mb-6 rounded-md bg-[#7c3aed] px-4 py-2 text-sm font-medium text-white hover:bg-[#6d28d9] transition"
      >
        Generate New Token
      </button>
      ...
      {/* Create form */}
      {showCreate && (
        <div className="mb-6 rounded-lg border border-[#334155] bg-[#1e293b] p-5 space-y-3 max-w-md">
          <div>
          ...
          <div>
            <label className="block text-sm text-[#94a3b8] mb-1">Expiration (days, leave empty for no expiry)</label>
            <input
              type="number"
              value={expiresInDays}
      ...
      <div className="space-y-3">
        {apiKeys.length === 0 ? (
          ...
          apiKeys.map((key) => (
            <div key={key.id} className="flex items-center justify-between rounded-lg border border-[#334155] bg-[#1e293b] px-4 py-3">
