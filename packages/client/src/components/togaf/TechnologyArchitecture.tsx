  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="p-3 border-b border-[#334155]">
        ...
        <p className="text-[10px] text-[#64748b] mt-1">ADM Phase D - Infrastructure, platforms, and technology standards</p>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-2 gap-2 p-3">
        <StatCard icon={<Cpu size={14} />} label="Tech Components" value={techComponents.length} color="#a855f7" />
      ...
      <div className="px-3 pb-3">
        <h4 className="text-[10px] font-semibold uppercase text-[#64748b] mb-2">Technology Radar</h4>
      ...
      {infrastructure.length > 0 && (
        <div className="px-3 pb-3 border-t border-[#334155] pt-3">
          <h4 className="text-[10px] font-semibold uppercase text-[#64748b] mb-2">Infrastructure</h4>
          {infrastructure.map((infra) => (
            <div key={infra.id} className="rounded-md border border-[#334155] bg-[#0f172a] p-2 mb-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <Server size={12} className="text-[#a855f7]" />
              ...
              <p className="text-[10px] text-[#64748b] mt-0.5">{infra.description || 'No description'}</p>
            </div>
          ))}
        </div>
      )}
...
function RadarQuadrant({ label, count, color, items }: { label: string; count: number; color: string; items: { name: string }[] }) {
  return (
    <div className="rounded-md border border-[#334155] bg-[#0f172a] p-2" style={{ borderTopColor: color, borderTopWidth: 2 }}>
      ...
      {items.length > 3 && <div className="text-[9px] text-[#475569]">+{items.length - 3} more</div>}
    </div>
  );
}
