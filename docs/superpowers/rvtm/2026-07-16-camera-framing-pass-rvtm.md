...
|----|-------------|-------|--------------|--------|----------|
| R-000 | AC-1 (real root cause): framing is **outlier-robust** — median centre + MAD-trimmed radius (`cutoff = medianDist + max(6·MAD, medianDist)`) so one broken layout position can't skew the centre or blow the distance | `ViewModeCamera.tsx` | `flyToStation.test.ts` (single outlier → bounded distance + centre stays in core) + browser JS diagnosis | PASS | live: on the reported project mean-centroid framing gave distance ~2700 (speck at edge); robust gives ~32. Data anomaly = THE-490 |
| R-001 | AC-1: all 6 stations frame the model at a readable size — `distFactor` band tightened to 1.5–1.9; Explore angle flattened from ~70° top-down to a 40° 3/4 view (`dir [0.5,0.6,0.5]`) | `ViewModeCamera.tsx` | `flyToStation.test.ts` (per-station framings differ) + user visual | PASS | user-confirmed 2026-07-16: "alle Stationen passen". vision 2.2→1.9, explore top-down→3/4, plan/govern 1.8→1.7, track 2.4→1.8; model 1.5 kept |
...
- **Real root cause found via live browser JS (2026-07-16):** on the reported project the model was a speck at the far left because ONE element ("Water and energy consumption tracking") has a broken `position3D` `(-366,13,-1682)` while the other 48 sit within ±24. Mean-centroid + raw maxDist → centre skewed to `(-6,8,-33)`, maxDist ~1687, camera distance ~2700. The distFactor tuning (R-001) barely helped; the **robust framing (R-000)** is the actual fix (distance ~32). Data anomaly ticketed as **THE-490**.
- **User visual — CONFIRMED (2026-07-16):** reloaded in the real browser (WebGL works there); model frames readable and centred beside the Sheet on all stations after the robust-framing fix; Explore flattened to a 3/4 view on request. User: "alle Stationen passen". Ready to merge.
