...
| R4 | 464 AC-4: Evidenz-Drilldown — Top-§§ (Titel+Nr) UND referenzierte Elemente, konsistent zum LAW-001-Drilldown | THE-464 | T3 + Spec-Review-Fix 1 | Component-Tests: §-Chips zeigen Titel (`keyParagraphDetails`, Tooltip=Key, Fallback=Key für Alt-Findings); Element-Chips lösen Ids über die Report-Signal-Evidenz zu Namen auf (Fallback: Id). Kette Judge→Persist→Merge→UI additiv (`keyParagraphDetails`), Merge-+lawJudge-Tests | Done |
...
| R7b | Review-Fix 1: corpus-only bekommt `workId` (`deriveNormWorkId`) + echtes `inPipeline` via World-State — sonst AC-5 unimplementierbar | Plan-Review | T3a + Spec-Review-Fix 4 | Merge-Tests (world) + Component-Test: confirmed corpus-only ⇒ Add-to-pipeline-Button sichtbar, Klick ruft `addToPipeline` mit workId (Test-Lücke geschlossen) | Done |
...
| R9 | 465 AC-2: Metriken jenseits Accuracy — P/R je Gesetz, Judge-Kalibrierung (ECE/Bands), FP/FN getrennt | THE-465 | T7 + Spec-Review-Fixes 2+3 | Runner-Tests + Report: `perFamilyBreakdown` (P/R/F2 je Familie, Retrieval UND End-to-End, Markdown+JSON); Judge-Kalibrierung nutzt die ECHTE Judge-Confidence (`buildJudgePostOutcome`) statt der Konstante 1 — ECE/Bands damit aussagekräftig (Bug gefixt) | Done |
