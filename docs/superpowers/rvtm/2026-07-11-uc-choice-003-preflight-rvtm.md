...
|---|---|---|---|---|---|
| R-442a | severity-Domain `low/medium/high/critical` + Migration bestandsdatenstabil | T1, T2, T4 | `shared/.../compliance.types.ts`, `models/Policy.ts`, `models/PolicyViolation.ts`, `scripts/migrate-severity-enforcement.ts` | `jest migrate-severity` (Mapping+Idempotenz) + `jest policy-evaluation -t THE-442` | **PASS** (2026-07-15: migrate-severity 3/3 inkl. rules.0-Guard + Orphan-Resolve; Commits ecb9e61/e2d06bf/9a86bdd) |
| R-442b | Scoring-Formel regressionsstabil (Alt-Score reproduziert) | T3 | `services/compliance.service.ts` | `jest compliance-score` (error·3+warning·1 ≙ high·3+medium·1 = 55%) | **PASS** (2026-07-15: compliance-score 3/3, 55%-Fall exakt; Reviewer-verifiziert term-für-term inkl. Clamp-Reihenfolge; Commit defc1b5) |
| R-442c | `ruleId` als stabile Rule-Identität + Index-Umstellung auf `(policyId,elementId,ruleId)` | T2, T4, T5 | `models/Policy.ts`, `models/PolicyViolation.ts`, `services/policy-evaluation.service.ts`, `routes/governance.routes.ts`, `routes/standards.routes.ts` | `jest policy-evaluation` (ruleId gesetzt, unique) | **PASS** (2026-07-15: diskriminierende Identity-Tests beide Richtungen; ensureRuleIds an ALLEN 3 Schreibgrenzen inkl. approve-policies-Fix 2ccc365; Stale-Rule-Cleanup; Commits e2d06bf/705e581/9165150/2ccc365) |
| R-442d | `enforcementLevel`-Achse (Default advisory), DTOs + LLM-Draft-Pipeline | T1, T2, T6 | `models/Policy.ts`, `shared/.../compliance.types.ts`, `standards.routes.ts`, `ai.service.ts` (Prompt-Sites 764/767/966) | Build 3 Pakete + `jest policy-evaluation` | **PASS** (2026-07-15: Default-advisory-Test grün; advisory hart gepinnt in approve-policies (weder LLM noch Request können eskalieren); critical dem LLM nicht angeboten; 3/3 Builds; Commits e2d06bf/ba2b188) |
| R-442e | Blast-Radius vollständig (Governance-UIs, Seeds, Tests) | T6, T7 | `client/.../PolicyDraftReview.tsx`, `ComplianceDashboard.tsx`, `PolicyManager.tsx`, `ui/PropertyPanel.tsx`, `ui/Sidebar.tsx`, `data/seed-policies.ts` | 2 `grep`-Sweeps (Legacy-Literale + Prompt-Prosa) + volle Suite | **PASS** (2026-07-15: beide Sweeps leer bzw. nur begründete Legacy-Tests/Fremd-Domänen; Server 1770/1773 (3 beweisbar vorbestehend), Client 217/217; „Enforcement severity"-Copy entwirrt; Commit ba2b188) |
| R-202a | Strukturierte Messages `{ruleId,severity,message,resourcePath,docLink}` + JSON-Schema | T8 | `schemas/validation-violation.schema.json`, `services/violation-format.ts` | `jest violation-schema` (ajv, CI-Gate) | **PASS** (2026-07-17: violation-schema 3/3 — ajv+ajv-formats; `enforcementLevel` in `required` (Review-Fix: Schema war laxer als Shared-Typ); Enum-Drift-Guard gegen `VIOLATION_SEVERITIES`/`ENFORCEMENT_LEVELS`; open-world dokumentiert; Commits d871f01/ea84e26) |
| R-202b | docLink auf Norm-Registry (Fallback KB), keine hartkodierten URLs | T9 | `services/policy-evaluation.service.ts` | `jest` deriveDocLink (standardId+section) | **PASS** (2026-07-17: policy-evaluation 60/60 — deriveDocLink inkl. ObjectId-Shape, encodeURIComponent-Fragment (`'Art. 5 (1)'`→`%20`, uri-reference-Kontrakt), positiver Persist-Pfad; beide Upsert-Sites verifiziert; KB-Fallback bewusst nicht gebaut (Plan: registry-first, sonst undefined → Client rendert keinen Link); Commits 3cf37cf/0b12420) |
| R-202c | Frontend: Inline-Violation-Details am 3D-Knoten (nicht nur Count) | T10 | `stores/complianceStore.ts`, `components/3d/NodeObject3D.tsx` | `vitest complianceStore.details` + In-Browser (Tooltip zeigt severity+message) | **PASS (unit)** (2026-07-17: complianceStore.details 3/3 — top-5 severity-sortiert + ruleId-Tiebreak, Referenz-Stabilität über Reloads (Object.is, Clone-Payload), el-2-Diskriminator, clear(); NodeObject3D: Top-3 `SEVERITY: message` + „+N more" (echter Gesamt-Count), 2-Zeilen-Clamp, nur Element-Knoten, X-Ray-suppressed (dokumentiertes Minimal-Label-Design); tsc clean; In-Browser-Smoke beim Deploy fällig (Präzedenz THE-442: Prod-verifiziert nach Merge); Commits dbf8901/a59d71c) |
...
**Coverage (Stand 2026-07-15, Chunk 1 ausgeführt):** 23 Requirements · **5 PASS** (R-442a–e) · 0 FAIL · 17 PENDING (Chunks 2–7) · 1 N/A.
...
- 2026-07-15 — **Chunk 1 AUSGEFÜHRT** (subagent-driven, Worktree javis-the442, Branch `mganzmanninfo/the-190-...`): 8 Code-Commits ecb9e61→2ccc365, 25+ Dateien, +900/−150. R-442a–e alle PASS (Evidenz in den Zeilen). 3 Review-Gates (nach T2, T4, T7) + Final-Review: 1 Important-Fix (approve-policies ensureRuleIds, 2ccc365). Execution-Funde über den Plan hinaus: advisor.service.ts + report.service.ts als checkCompliance-Konsumenten, Stale-Rule-Cleanup (Violations gelöschter Rules), Runbook-Fix (dist-Pfad statt ts-node), dropIndex-Catch verengt, temporärer ruleId-Default (Task-2-Interim, in Task 5 entfernt). Deploy ausstehend — Tasks 2–5 = EIN Deploy-Unit, Migration sofort nach Deploy vor Traffic.
