...
- **Heatmap auto-enable is now data-gated (`8e79219`):** entering a conformance station only lights the heatmap when the project has coverage data (`mappingsByElement.size > 0`); an unassessed project (0 mappings) stays neutral instead of showing a misleading wall of red "gaps". +1 test (unassessed → glow stays off). The deeper semantic — 0-mapping elements should read "not assessed" rather than red "gap" inside `ComplianceGlow` itself — is **THE-489** (shared THE-281 code, own ticket).
- **User to visually confirm:** AC-2 ComplianceGlow coverage heatmap on a project **with** mappings (needs real coverage data; the automation tab's WebGL renders black), and that leaving a conformance station restores the prior glow (AC-7 restore-on-leave).
